...
// Updated and cleaned: integrates algo.dart ranking, caching, feed-mode toggle,
// reduces icons in feed actions and moves less-important actions into menu.
...
// import algorithm (adjust path if your folder structure differs)
import 'algo.dart';
    ...
    try {
      ...
      final newPosts = snapshot.docs.map((doc) {
        ...
        return {
          ...
          'commentsCount':
              (data['commentsCount'] is int) ? data['commentsCount'] as int : 0,
          'views': (data['views'] is int) ? data['views'] as int : 0,
          'tags': data['tags'] ?? [],
          'followerCount':
              (data['followerCount'] is int) ? data['followerCount'] as int : 0,
...
class PostCardWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final userRecord = users.firstWhere((u) => (u['id'] as String?) == userId,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
              ...
              trailing: PopupMenuButton<String>(
                onSelected: (value) {
                  if (value == 'report') {
                    ...
                    Clipboard.setData(ClipboardData(text: post['post'] ?? ''))
                        .then((_) {
                      ScaffoldMessenger.of(context).showSnackBar(
                ...
                itemBuilder: (_) => const [
                  ...
                  PopupMenuItem(
                      value: 'send', child: Text('Send / Start Watch Party')),
            ...
            if (media.isNotEmpty)
              Padding(
                ...
                child: ConstrainedBox(
                  constraints:
                      BoxConstraints(maxHeight: imageHeight, minHeight: 120),
                  child: ClipRRect(
            ...
            // Actions row (reduced icons — less important actions moved to menu)
            Padding(
...
class _SocialReactionsScreenState extends State<_SocialReactionsScreen>
  ...
  String _feedMode =
      'for_everyone'; // 'for_everyone' | 'trending' | 'fresh' | 'personalized'
  ...
  Future<void> _initializeData() async {
    try {
      ...
      // refresh ranked cache once initial posts fetched
      _refreshRankedCache();
    } catch (e) {
  ...
  // Simple push to PostReviewScreen — the screen will handle upload/save if no callback is provided.
  ...
  List<Map<String, dynamic>> _rankAndApply(List<Map<String, dynamic>> posts) {
    try {
      final ranked = Algo.rankPosts(posts,
          ...
          mode: _feedMode,
          seed: DateTime.now().millisecondsSinceEpoch % 100000 // small entropy
          );
      return ranked;
    } catch (e) {
  ...
  // returns cached ranked list, recomputes if source or mode changed
  ...
  // call this after fetches if you want to proactively refresh the cache
        ...
        Expanded(
          child: RefreshIndicator(
            onRefresh: () async {
              ...
              // refresh cached ranking when fetch completes
              _refreshRankedCache();
            },
                ...
                SliverList(
                  ...
                  return RepaintBoundary(
                    child: PostCardWidget(
                      ...
                      // Watch party now pushes to a dedicated screen (watch_party_screen.dart)
                      onWatchParty: (post) {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                      ...
                      onSend: (post) {
                        // moved from icon to menu — kept as lightweight "start watch party / send"
                        final code =
                            (100000 + Random().nextInt(900000)).toString();
                        if (mounted)
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content:
                                  Text('Started Watch Party: Code $code')));
                        setState(() {
                          _notifications.add(
                      ...
                      onShare: (post) async {
                        try {
                          ...
                          // If there's a media url, include it
                          if (post['media'] != null &&
                      ...
                      onRetweet: (post) async {
                        ...
                        try {
                          ...
                          // increment retweet count on original if possible
        ...
        child: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            final stories = snapshot.data!.docs.map((doc) {
              ...
              try {
                return DateTime.now()
                        .difference(DateTime.parse(s['timestamp'])) <
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (s) {
              setState(() {
                _feedMode = s;
                // recompute ranked posts immediately
                _refreshRankedCache();
              });
            },
            itemBuilder: (_) => [
              ...
              child: Container(
                  decoration: BoxDecoration(
                      gradient: RadialGradient(
                          ...
                          radius: 1.6,
                          colors: [
                            widget.accentColor.withAlpha((0.2 * 255).round()),
                            Colors.transparent
                          ],
                          stops: const [
                            0.0,
